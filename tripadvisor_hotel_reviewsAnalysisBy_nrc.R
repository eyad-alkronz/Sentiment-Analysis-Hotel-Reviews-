...
#to show words count by sentiments that offers by bing
...
nrc <- get_sentiments("nrc") %>%
  ...
  mutate(value = ifelse(sentiment == "anger" , 0 , 
                        ifelse(sentiment == "disgust" , 0 , 
                               ifelse(sentiment == "fear" , 0 , 
                                      ifelse(sentiment == "negative" , 0 , 
                                             ifelse(sentiment == "sadness" , 0 , 
                                                    ifelse(sentiment == "positive" , 1 , 
                                                           ifelse(sentiment == "surprise" , 1 , 
                                                                  ifelse(sentiment == "trust" , 1 , 
                                                                         ifelse(sentiment == "joy" , 1 , 
                                                                                ifelse(sentiment == "anticipation" , 1 , 1)))))))))
                        ) )   
...
#Enhancy accuracy
data%>%filter(differ!=0) %>% group_by(Rating , newRating , result) %>% summarize(count = n()) %>% print()
